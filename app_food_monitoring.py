...
st.set_page_config(page_title="NutriCam", page_icon="🥗", layout="wide")
        ...
        if menus:
            for row in menus:
                ...
                with st.expander(f"{icon} {mtype.capitalize()} — {mitems} — {mcal} kcal", expanded=False):
                    ...
                    if st.button(f"🗑️ Delete {mtype.capitalize()}", key=f"del_menu_{mid}"):
...
def render_login():
    st.title("🥗 NutriCam — AI Calorie Tracker")
    st.subheader("Your personal AI nutrition + hostel companion")
    ...
    with tab2:
        ...
        with col2:
            ...
            target = st.number_input("Daily Calorie Target", 1000, 5000, 2000)
...
def render_student_app():
    ...
    with st.sidebar:
        ...
        if st.session_state.water_count >= water_goal:
            ...
            st.warning(f"⚠️ {water_goal - st.session_state.water_count} more glasses needed!")
        st.divider()
    ...
    col3.metric("⚖️ Net",     f"{net_calories} kcal", f"{remaining} remaining")
    col4.metric("💪 Protein", f"{total_protein:.1f}g")
    col5.metric("💧 Water",   f"{st.session_state.water_count}/8")

    if net_calories > target:
        st.error(f"⚠️ You exceeded your calorie goal by {net_calories - target} kcal!")
    elif net_calories > target * 0.9:
        st.warning("⚠️ You are close to your daily calorie limit!")
    else:
    ...
    tab1, tab2, tab3, tab4, tab5, tab6, tab7, tab8 = st.tabs([
        ...
        "⚖️ Weight", "🤖 AI Chatbot", "📥 Export",
        "🍽️ Mess Menu", "💬 My Queries"
    ])
    ...
    with tab1:
        if meals:
            ...
            for meal in meals:
                with st.expander(f"🍽️ {meal[2]} — {meal[3]} kcal ({meal[7]})"):
                    mc1, mc2, mc3 = st.columns(3)
                    mc1.write(f"💪 Protein: {meal[4]}g")
                    mc2.write(f"🌾 Carbs:   {meal[5]}g")
                    mc3.write(f"🧈 Fats:    {meal[6]}g")
                    if st.button(f"🗑️ Delete", key=f"del_{meal[0]}"):
                        delete_meal(meal[0])
                        st.rerun()
            ...
            if uploaded_file is not None:
                ...
                if st.button("🤖 Analyze Uploaded Image", type="primary"):
                    with st.spinner("AI is analyzing your food…"):
                        ...
                        st.success(f"🍽️ Detected: **{data.get('name', 'Unknown Food')}**")
    ...
    with tab6:
        ...
        with exp_c1:
            ...
            st.write(f"- ⚖️ Net calories: **{net_calories} kcal**")
    ...
    # ── Tab 7 : Mess Menu ─────────────────────────────────────────────────────
    with tab7:
        st.subheader("🍽️ Today's Hostel Mess Menu")
        ...
        if today_menu:
            ...
            st.success(f"🍽️ Total mess calories today: **{total_mess_cal} kcal**")
